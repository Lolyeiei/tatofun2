...
if (isset($_POST['register'])) {
    ...
    $phone = mysqli_real_escape_string($conn, $_POST['phone']); // รับค่าเบอร์โทร
    $address = mysqli_real_escape_string($conn, $_POST['address']); // รับค่าที่อยู่
    $role = 'member';
    ...
    if (mysqli_num_rows($query_check) > 0) {
        ...
        // 2. บันทึกลงตาราง tb_users (เพื่อสร้าง User ID สำหรับ Login)
        $sql1 = "INSERT INTO tb_users (username, password, role) VALUES ('$username', '$password', '$role')";
        
        if (mysqli_query($conn, $sql1)) {
            // 3. ดึง ID ล่าสุดที่เพิ่งสร้างจากตาราง tb_users
            $last_id = mysqli_insert_id($conn);

            // 4. บันทึกลงตาราง tb_customer_profile โดยใช้ ID จากข้อ 3

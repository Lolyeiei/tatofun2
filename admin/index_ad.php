...
<body>
    ...
    <div class="container-fluid px-4 mt-3">
        <?php
        $sql_note = "SELECT * FROM tb_notifications WHERE status = 'unread' ORDER BY created_at DESC";
        $res_note = mysqli_query($conn, $sql_note);

        if(mysqli_num_rows($res_note) > 0) {
            while($note = mysqli_fetch_assoc($res_note)):
        ?>
        <div class="alert alert-danger shadow-lg border-0 d-flex align-items-center mb-3 p-3 animate__animated animate__headShake" role="alert" 
             style="border-radius: 15px; background: linear-gradient(90deg, #fff5f5 0%, #fff 100%); border-left: 8px solid #dc3545 !important;">
            
            <div class="flex-shrink-0 bg-danger text-white rounded-circle d-flex align-items-center justify-content-center shadow-sm pulse-icon" style="width: 45px; height: 45px;">
                <i class="bi bi-megaphone-fill fs-5"></i>
            </div>
            
            <div class="flex-grow-1 ms-3">
                <div class="d-flex justify-content-between align-items-center">
                    <strong class="text-danger fs-5">ประกาศด่วนจากระบบสต็อก</strong>
                    <small class="text-muted fw-bold bg-light px-2 py-1 rounded-pill" style="font-size: 0.75rem;">
                        <i class="bi bi-clock me-1"></i><?= date('H:i', strtotime($note['created_at'])) ?> น.
                    </small>
                </div>
                <div class="text-dark opacity-75 mt-1"><?= htmlspecialchars($note['message']) ?></div>
            </div>

            <div class="ms-3">
                <a href="mark_read.php?id=<?= $note['id'] ?>" class="btn btn-sm btn-danger rounded-pill px-3 shadow-sm fw-bold">
                    รับทราบแล้ว
                </a>
            </div>
        </div>
        ...
        <div class="row g-4 justify-content-center">
            ...
            <div class="col-12 col-md-6 col-lg-4">
                <div class="card admin-card p-4 text-center border-top border-5 border-dark">
                    <div class="card-body d-flex flex-column p-0">
                        <div class="icon-circle bg-dark bg-opacity-10 text-dark"><i class="bi bi-graph-up-arrow"></i></div>
                        <h4 class="fw-bold">สรุปรายได้</h4>
                        <p class="text-muted small">ดูสถิติรายได้รายวัน และยอดขายสะสม</p>
                        <div class="mt-auto pt-4">
                            <a href="sales_report.php" class="btn btn-dark w-100 btn-action shadow-sm"><i class="bi bi-file-earmark-bar-graph me-2"></i>ดูรายงานสถิติ</a>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="col-12 col-md-6 col-lg-4">
                <div class="card admin-card p-4 text-center border-top border-5 border-danger">
                    <div class="card-body d-flex flex-column p-0">
                        <div class="icon-circle bg-danger bg-opacity-10 text-danger"><i class="bi bi-megaphone"></i></div>
                        <h4 class="fw-bold">จัดการโปรโมชั่น</h4>
                        <p class="text-muted small">สร้างแบนเนอร์ และกิจกรรมพิเศษหน้าแรก</p>
                        <div class="mt-auto pt-4">
                            <a href="manage_promotion.php" class="btn btn-danger w-100 btn-action shadow-sm text-white"><i class="bi bi-percent me-2"></i>เข้าสู่หน้าจัดการ</a>
                        </div>
                    </div>
                </div>
            </div>

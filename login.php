...
<head>
    ...
    <style>
        ...
        body {
            ...
            background-color: #ffca28; /* สีกันตายกรณีโหลดรูปไม่ขึ้น */
            overflow: hidden;
            position: relative;
        }

        /* --- พื้นหลังเนียนๆ ป้องกันลายซ้ำ --- */
        .bg-gradient-layer {
            position: absolute;
        ...
        .floating-potato {
            ...
            pointer-events: none; /* ห้ามเมาส์ไปโดน */
            transition: transform 15s linear, opacity 15s linear;
        }

        /* --- กล่อง Login สไตล์ Glassmorphism --- */
        .login-card {
            width: 100%;
            max-width: 420px;
            padding: 40px;
...
<body>
    ...
    <div id="potato-container"></div> <div class="login-card">
    ...
    <script>
        function spawnPotato() {
            ...
            // ใช้รูปมันฝรั่งของคุณเอง (Path ถูกต้องแน่นอน)
            const imgUrl = 'admin/img_ad/LOGO3.png'; 
            
            // สุ่มขนาด และใส่รูป
            const size = Math.random() * (90 - 45) + 45;
            potato.innerHTML = `<img src="${imgUrl}" style="width: ${size}px; opacity: 0.8;">`;
            
            // สุ่มตำแหน่งเริ่มต้น (ด้านล่างจอ)
            potato.style.left = Math.random() * 100 + 'vw';
            potato.style.bottom = '-100px';
            
            container.appendChild(potato);

            // ใช้ระบบ Transition แทน Animation เพื่อความลื่นไหล
            setTimeout(() => {
                const randomX = (Math.random() - 0.5) * 200; // สุ่มการส่ายซ้ายขวา
                potato.style.transform = `translate(${randomX}px, -120vh) rotate(${Math.random() * 360}deg)`;
                potato.style.opacity = '0';
            }, 100);

            // ลบรูปทิ้งเมื่อลอยพ้นจอ
            setTimeout(() => { potato.remove(); }, 15000);
        }

        // ปล่อยมันฝรั่งออกมาทุกๆ 2.5 วินาที
        setInterval(spawnPotato, 2500);
        // เริ่มต้นปล่อยทันที 6 ตัวให้หน้าจอไม่ว่างเกินไป
        for(let i=0; i<6; i++) {
            setTimeout(spawnPotato, i * 500);
        }
    </script>
